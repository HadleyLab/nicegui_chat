...
class TestChatService:
    ...
    @pytest.fixture
    def chat_service(
        self, mock_auth_service, mock_memory_service, mock_config, mock_agent
    ):
        """Create ChatService instance."""
        return ChatService(
            mock_auth_service, mock_memory_service, mock_config, mock_agent
        )
...
class TestMemoryService:
    ...
    @pytest.mark.asyncio
    async def test_search_not_authenticated_duplicate(
...
class TestAgentService:
    ...
    async def test_generate_stream_success(self, agent_service):
        ...
        with patch.object(agent_service._agent, "run_stream") as mock_run_stream:
            ...
            mock_run_stream.return_value.__aenter__ = AsyncMock(
                return_value=mock_result
            )
            mock_run_stream.return_value.__aexit__ = AsyncMock(return_value=None)

            events = []
            async for event_type, data in agent_service.generate_stream(
                conversation, "Hello"
            ):
    ...
    async def test_generate_stream_with_exception(self, agent_service):
        ...
        with patch.object(agent_service._agent, "run_stream") as mock_run_stream:
            ...
            mock_run_stream.return_value.__aenter__.side_effect = Exception(
                "Test error"
            )
            mock_run_stream.return_value.__aexit__ = AsyncMock(return_value=None)
