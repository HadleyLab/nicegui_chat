...
class ChatUI:
        ...
        with self.chat_container:
            with ui.card().props("bordered"):
                with ui.row().classes("items-center gap-3"):
                    ui.html(
                        f'<img src="{self.config.ui.logo_icon_path}" style="height: 32px; width: auto;" />'
                    )
            ...
            with ui.row().classes("w-full items-center justify-between"):
                ...
                with ui.row().classes("gap-2"):
                    ...
                    ui.button(icon="refresh", on_click=self._new_conversation).props(
                        "flat round"
                    ).tooltip(self.config.ui.new_conversation_tooltip)

                    ui.button(icon="logout", on_click=self._logout).props(
                        "flat round"
                    ).tooltip(self.config.ui.logout_tooltip)
        ...
        with ui.card().classes("w-full").props("flat"):
            with ui.row().classes("w-full p-2 gap-3 items-center max-w-4xl mx-auto"):
                ...
                ui.button(icon="send", on_click=self._send_message).props(
                    "round color=primary"
                ).tooltip(self.config.ui.send_tooltip)
    ...
    def _logout(self) -> None:
        ...
        ui.notify(self.config.ui.logout_notification, type="info", position="top")
